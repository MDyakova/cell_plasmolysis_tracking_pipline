                ...
                for j in range(0, image.shape[2], tile_size-50):
                    ...
                    if (file_name not in processed_files):
                        ...
                        if len(df_cells)>=len(all_masks_save):
                            # Track using trackpy
                            try:
                                tracked = tp.link_df(df_cells, search_range=20, memory=2)
                                ...
                                if len(pd.unique(tracked['label']))==0:
                                    tracked = tp.link_df(df_cells, search_range=30, memory=2)
                                    # Remove short-lived tracks
                                    tracked = tp.filter_stubs(tracked, threshold=len(all_masks_save))
                                    all_masks_save = np.stack(all_masks_save)
                                    tracked.reset_index(drop=True, inplace=True)
                            ...
                            if np.max(all_masks_save>0)>0:
                                ...
                                video = (image[:, i:i+tile_size, j:j+tile_size]
                                            /image[step][i:i+tile_size, j:j+tile_size].max()*255).astype('uint8')
                                tiff.imwrite(os.path.join(output_directory, 
                        ...
                        with open(os.path.join(output_directory, 'processed_files.txt'), 'a') as f:
                            f.write(f'{file_name_save}_x_{str(j)}_y_{str(i)}.tif' + '\n')
